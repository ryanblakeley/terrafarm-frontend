...
class SearchResultsForResources extends React.Component {
  static propTypes = {
  ...
  render () {
    ...
    return <div>
      ...
      <BrowseContentWrapper
            ...
            panels={[
              {
                ...
                body: <RelationshipList
                  listItems={query.searchResources.edges.map(edge => ({
                    name: edge.node.name,
        ...
        map={<SearchMap
          places={query.searchResources.edges.map(edge => ({
            name: edge.node.name,
            position: parseCoords(edge.node.placeByPlaceId.coords),
          }))}
          onChangeSearchBounds={this.changeSearchBounds}
        />}
      />
    </div>;
  }
...
export default Relay.createContainer(SearchResultsForResources, {
  initialVariables: {
    count: 5,
    search: '',
