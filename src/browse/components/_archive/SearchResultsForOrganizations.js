...
class SearchResultsForOrganizations extends React.Component {
  static propTypes = {
  ...
  render () {
    ...
    return <div>
      <ContentHeader text={'Organizations'} />
      <BrowseContentWrapper
            ...
            panels={[
              {
                ...
                body: <RelationshipList
                  listItems={query.searchOrganizations.edges.map(edge => ({
                    name: edge.node.name,
        ...
        map={<SearchMap
          places={query.searchOrganizations.edges.map(edge => ({
            name: edge.node.name,
...
export default Relay.createContainer(SearchResultsForOrganizations, {
  initialVariables: {
    count: 5,
    search: '',
