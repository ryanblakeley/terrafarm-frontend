...
export default mutationWithClientMutationId({
  ...
  outputFields: {
    landEdge: {
      ...
      resolve: async ({localResourceId, localLandId}) => {
        ...
        const landPromises = resource.lands.map(g => getItem(landEndpoint, g.id));
    ...
    resourceEdge: {
      type: ResourceEdge,
      resolve: async ({localLandId, localResourceId}) => {
        const land = await getItem(landEndpoint, localLandId);
        const resourcePromises = land.resources.map(r => getItem(resourceEndpoint, r.id));
  ...
  mutateAndGetPayload: async ({resourceId, landId}) => {
    ...
    resource.lands.push({id: localLandId});
