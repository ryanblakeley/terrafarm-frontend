...
export default mutationWithClientMutationId({
  ...
  outputFields: {
    projectEdge: {
      ...
      resolve: async ({localResourceId, localProjectId}) => {
        ...
        const projectPromises = resource.projects_pending.map(g => getItem(projectEndpoint, g.id));
    ...
    resourceEdge: {
      ...
      resolve: async ({localResourceId, localProjectId}) => {
        ...
        const resourcePromises = project.resources_pending.map(r => getItem(resourceEndpoint, r.id));
  ...
  mutateAndGetPayload: async ({resourceId, projectId}) => {
    ...
    const resource = await getItem(resourceEndpoint, localResourceId);
    resource.projects_pending.push({id: localProjectId});
