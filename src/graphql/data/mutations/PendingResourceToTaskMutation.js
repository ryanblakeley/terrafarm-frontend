...
export default mutationWithClientMutationId({
  ...
  outputFields: {
    taskEdge: {
      ...
      resolve: async ({localResourceId, localTaskId}) => {
        ...
        const taskPromises = resource.tasks_pending.map(g => getItem(taskEndpoint, g.id));
    ...
    resourceEdge: {
      ...
      resolve: async ({localResourceId, localTaskId}) => {
        ...
        const resourcePromises = task.resources_pending.map(r => getItem(resourceEndpoint, r.id));
  ...
  mutateAndGetPayload: async ({resourceId, taskId}) => {
    ...
    const resource = await getItem(resourceEndpoint, localResourceId);
    resource.tasks_pending.push({id: localTaskId});
